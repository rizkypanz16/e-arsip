	...
	<div class="content-wrapper">
		...
		<section class="content">
			<nav aria-label="breadcrumb">
				<ol class="breadcrumb">
					<li class="breadcrumb-item active" aria-current="page"><i class="fas fa-envelope text-dark mr-2"></i><strong class="text-dark">  Data Surat Keluar</strong></li>
				</ol>
			</nav>
			<div class="container-fluid">
				<div class="row">
					<div class="col-12">
						<div class="card col-8">
							<div class="card-header py-2">
								<strong class="card-title font-weight-bold">Perhatian</strong>
							</div>
							<div class="card-body py-2">
								<p class="card-text">* <b>Memo</b> / <b>Nota</b> untuk unit tujuan <i>internal</i>. <br>
								* <b>Surat</b> untuk unit tujuan <i>eksternal</i>. <br>
								* File yang sudah di upload tidak bisa dihapus kembali. <br>
								* Untuk <b>Unit Tujuan</b> <i>internal</i> atau <b>Nama Pegawai</b> yang belum terdaftar harap hubungin bagian Administrator.</p>
							</div>
						</div>
					</div>
					<div class="col-12">
						<div class="card">
							<div class="card-header">
								<button type="button" class="btn btn-xs btn-danger" onclick="show_modal()">
									<i class="fa fa-plus"></i> Dokumen Keluar
								</button>
							</div>
							<div class="card-body">
								<table class="table table-bordered table-hover" id="dataTable1" style="width: 100%;">
									<thead>
										<tr>
											<th>No. </th>
											<th>Jenis Dokumen</th>
									...
									<tbody>
										<?php 
										$no = 1;
										foreach($keluar_dok as $values){
										?>
											<tr style="font-size: 13px; ">
												<td><?= $no++ ?></td>
												<td>
													<?php
														switch($values->jns_dokumen){
															case 1:
																echo "Memo";
																break;
															case 2:
													...
													?><br>
													<?php
														switch($values->kategori){
															case 1:
																echo '<span class="badge badge-success"> Umum</span>';
																break;
															case 2:
															...
															case 3:
																...
																break;
															case 4:
																echo '<span class="badge badge-danger"> Confidential</span>';
																break;
															case 5:
																echo '<span class="badge badge-danger"> Private & Confidential</span>';
																break;
														}
													?>
												</td>
												<td><strong><?= "Perihal : ". $values->perihal ?></strong><hr>Pembuat : <?= $values->nm_pegawai ?><br> Tujuan : <?= substr($values->unit_tujuan, 15, -3) ?> <br>No Surat : <?= $values->no_dokumen ?></td>
												<td><?= $values->sts_dokumen ?></td>
												<td><?= "Tgl Dibuat : ". substr($values->tgl_dokumen, 0, 10) ?></td>
												<td>
													<a href="<?= base_url('assets/' . $values->path_folder . '/' . $values->file_dokumen) ?>" class="badge badge-info"><i class="fa fa-download"></i></a>
													<button class="badge badge-warning" style="cursor: pointer" onclick="printJS('<?= base_url('assets/' . $values->path_folder . '/' . $values->file_dokumen) ?>')"><i class="fa fa-print"></i></button>
													<span class="badge badge-info" style="cursor: pointer" onclick="view(<?= $values->id_dokumen ?>)"><i class="fa fa-eye"></i></span>
													<span class="badge badge-success" style="cursor: pointer" onclick="sunting(<?= $values->id_dokumen ?>)"><i class="fa fa-edit"></i></span>
													<span class="badge badge-danger" style="cursor: pointer" onclick="hapus(<?= $values->id_dokumen ?>)"><i class="fa fa-trash"></i></span>
												</td>
											</tr>
										<?php } ?>
									</tbody>
			...
			<div class="modal-body">
				<form id="form" enctype="multipart/form-data" autocomplete="off">
					...
					<div class="form-group row">
						...
						<div class="col-sm-6">
							<input type="text" class="form-control" name="no_surat" id="no_surat" hidden>
						</div>
					</div>
					<div class="form-group row">
						<label class="col-sm-2 col-form-label">Tanggal Dokumen</label>
						<div class="col-sm-6">
							<input type="date" class="form-control" name="tgl_dokumen" id="tgl_dokumen">
						</div>
					</div>
					<div class="form-group row">
					...
					<div class="form-group row">
						<div class="offset-2 col-sm-6">
							<button type="submit" class="btn btn-danger btn_save"></button>
