...
class Laporan_dok_keluar extends CI_Controller
{
	...
	public function detail_keluar(){
		...
		$query2 = "SELECT * FROM tbl_dok_keluar WHERE month(tgl_dokumen)=".$bulan." and year(tgl_dokumen)=".$tahun;
		$data = $this->db->query($query2)->result();
		// echo '<pre>' , var_dump($data) , '</pre>';die;
		$this->load->view('user/v_detail_keluar.php', [
			'data' => $data,
			'bulan' => $bulan,
			'tahun' => $tahun
		]);
