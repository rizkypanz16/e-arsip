...
class Dokumen_keluar extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['keluar_dok'] = $this->db->from('tbl_dok_keluar')
			->join('tbl_pegawai', 'tbl_pegawai.id_pegawai = tbl_dok_keluar.pembuat')
			->get()->result();
	...
	public function get_list()
	{
		...
		foreach ($list as $li) {
			...
			$aksi .= '<a href="#" onclick="printJS('. base_url('assets/' . $li['path_folder'] . '/' . $li['file_dokumen']) .')" class="badge badge-success"><i class="fas fa-print"></i></a>&nbsp;';
	...
	public function insert()
	{
		...
		elseif ($no > 9) $cond = '0' . $no;
		else $cond = '0' . $no;
		$month = array('I', 'II', 'III', 'IV', 'V', 'VI', 'VII', 'VIII', 'IX', 'X', 'XI', 'XII');
		$bulan = substr(input('tgl_dokumen'), 5, 2);
		$tahun = substr(input('tgl_dokumen'), 0, 4);
		// set tahun terbitan
		$no_dok = substr($config['thn_dokumen'], 2, 1) - 2 . substr($config['thn_dokumen'], -1);
		// set counter nomor dokumen & kode dokumen
		// $no_dok .= '421.2/' . $cond . '-' . $dokumen['id_jns_dokumen'];
		$no_dok .= '421.2/' . $cond;
		// set kode group
		$no_dok .= '.-SDN.SKM.09/'.$month[(int)$bulan-1].'/'.$tahun;

		$data = array(
			'no_dokumen' => substr($no_dok, 2),
